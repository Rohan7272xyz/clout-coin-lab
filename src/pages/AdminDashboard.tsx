...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const statsResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/dashboard/admin/stats`, {
      ...
      // Fetch users
      const usersResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/dashboard/admin/users`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      // Fetch influencers
      const influencersResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/dashboard/admin/influencers`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const approveInfluencer = async (influencerId: number) => {
    try {
      const token = await user?.getIdToken();
      if (!token) return;

      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/dashboard/admin/influencers/${influencerId}/approve`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const rejectInfluencer = async (influencerId: number) => {
    try {
      const token = await user?.getIdToken();
      if (!token) return;

      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/dashboard/admin/influencers/${influencerId}/reject`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
